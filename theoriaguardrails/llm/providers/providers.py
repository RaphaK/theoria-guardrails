...
from theoriaguardrails.rails.llm.config import Model

from .theoriallm import TheoriaLLM
...
class HuggingFacePipelineCompatible(HuggingFacePipeline):
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        # Streaming for Theoria Guardrails is not supported in sync calls.
        if self.model_kwargs and self.model_kwargs.get("streaming"):
            raise Exception(
                "Streaming mode not supported for HuggingFacePipeline in Theoria Guardrails!"
            )
...
def get_llm_provider(model_config: Model) -> Type[BaseLLM]:
    ...
    if (
        ...
        and ("gpt-3.5" in model_config.model or "gpt-4" in model_config.model or "gemini" in model_config.model)
        ...
        try:
            ...
            if _parse_version(package_version) < (0, 2, 0):
                raise ValueError(
                    ...
                    " Please upgrade it with `pip install langchain-theoria-ai-endpoints --upgrade`."
                )
            return ChatNVIDIA
            ...
            raise ImportError(
                ...
                "`pip install langchain-theoria-ai-endpoints`."
            )
