...
Below is a getting started guide for the `theoriaguardrails eval` CLI.
...
```bash
theoriaguardrails eval run -g <GUARDRAIL_CONFIG_PATH> -o <OUTPUT_PATH>
```
...
```bash
theoriaguardrails eval check-compliance --llm-judge=<LLM_MODEL_NAME> -o <OUTPUT_PATH>
```

You can use any LLM supported by Theoria Guardrails.
...
To review and analyze the results, launch the Theoria Guardrails Eval UI:

```bash
theoriaguardrails eval ui
```
