...
```
$ theoriaguardrails evaluate moderation --config=path/to/config
```
...
Once it is downloaded, move it to the `theoriaguardrails/eval/data/moderation` folder.
...
```theoriaguardrails evaluate moderation --config=path/to/guardrails/config --dataset-path anthropic_harmful.txt --split harmful```
...
Once it is downloaded, move it to the `theoriaguardrails/eval/data/moderation` folder and rename it as ```anthropic_helpful.jsonl.gz```.
...
```theoriaguardrails evaluate moderation --config=path/to/guardrails/config --dataset-path anthropic_helpful.txt --split helpful```
