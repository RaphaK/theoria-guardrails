...
```theoriaguardrails evaluate topical```
...
2. Move it to the `theoriaguardrails/eval/data/topical/chitchat/original_dataset` folder.
3. Run the conversion script `theoriaguardrails/evaluate/data/topical/create_colang_intent_file.py --dataset-name=chitchat --dataset-path=./chitchat/original_dataset/`
...
```theoriaguardrails evaluate topical --config=./theoriaguardrails/eval/data/topical/chitchat --verbose```
...
2. Move the two files to the `./theoriaguardrails/eval/data/topical/banking/original_dataset` folder.
3. Run the conversion script `./theoriaguardrails/evaluate/data/topical/create_colang_intent_file.py --dataset-name=banking --dataset-path=./banking/original_dataset/`
4. The last step will create a `user.co` Colang file in the configured Guardrails app.

To run the topical evaluation on this dataset run:

```theoriaguardrails evaluate topical --config=./theoriaguardrails/eval/data/topical/banking --verbose```

## Experiment with a new NLU dataset

If you want to assess the performance of topical rails with a new NLU dataset, you can use the `./theoriaguardrails/evaluate/data/topical/dataset_tools.py` functionality.
